							...
							////***********************************************************************************************************************************************************************************************
								if(turnoColor == true && tablero[fichaOrigen[0]][fichaOrigen[1]].getColor().equals("blanca" )  || turnoColor == false && tablero[fichaOrigen[0]][fichaOrigen[1]].getColor().equals("negra") )  { // Comprobacion de que efectivamente el color de la ficha y el del turno coinciden
									...
									if(poderMover(fichaDestino, turnoColor)){//metodo que devuelve si se puede realizar el movimiento desde el punto de vista de la tabla (la casilla de destino existe y esta vacia u la ficha es del tipo contrario...)
										...
										else if(tablero[fichaOrigen[0]][fichaOrigen[1]] instanceof Peon){
											System.out.println("Cambio en else if statement, comprobacion de que en caso de movimient devuelva false, se ejecuta mov peon si es objeto es peon");
											if(movimientoSoloPeon(fichaOrigen[0] ,fichaOrigen[1], fichaDestino[0], fichaDestino[1], tablero[fichaOrigen[0]][fichaOrigen[1]].getColor() , ((Peon) tablero[fichaOrigen[0]][fichaOrigen[1]]).isPrimerMovimiento() ))   { // en caso de comprobacio de posibilidad de movimient pero especificamente con PEON 
												...
												// if else statement que compruena si la ficha que realiza un movimiento es un peon.
													...
													borradorFichaOrigen(fichaOrigen);
													mostrarPosiciones();
													// Llamar a metodo que compruebe si el peon esta en la ultima linea , en caso de ser asi mostrar mensaje para cambiar ficha.
											}
											else {
												System.out.println("Algun error con el movimiento de peon. ");
...
///V/////////////++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++	
		...
		if(yOrigen != yDestino && xOrigen == xDestino) { // 1º if else statement se comprueba en movimient lineal que solo se modifica eje de x || y
			if(yOrigen > yDestino) { //if statement una vez comprobado que solo se modifica un eje vemos si el eje y es mayor o menor que la coordenada y de destino en caso de origen > destino se compruenba en posicion origen - 1  
				...
				while(yOrigen > yDestino) { // si el nombre de esa coordenada es Casilla en caso de no serlo sabremos que hay una ficha contraria u nuestra, devolveremos false en ese caso
					if(tablero[yOrigen - 1][xOrigen].getNombre().equals("Casilla") == false && yOrigen - 1 != yDestino) {
						return false;
					}
			...
			if(xOrigen > xDestino) {
				while(xOrigen > xDestino) {
					...
					if(tablero[yOrigen][xOrigen - 1].getNombre().equals("Casilla") == false && xOrigen - 1 != xDestino) {
						return false;
	...
	public boolean movimientoDiagonal(int yOrigen, int xOrigen, int yDestino, int xDestino){
    	
		if(yOrigen > yDestino && xOrigen > xDestino) {
			while(yOrigen > yDestino && xOrigen > xDestino) {
				if(tablero[yOrigen -1][xOrigen - 1].getNombre().equals("Casilla") == false  && yOrigen -1 != yDestino && xOrigen -1 != xDestino) {
					return false;
...
//public static boolean movimientoLineal(int yOrigen, int xOrigen, int yDestino, int xDestino, String nombre){}
  ...
  public boolean movimientoSoloPeon(int yOrigen, int xOrigen, int yDestino, int xDestino, String color,boolean primerMovimiento){	 
	  int yR = 0, xR = 0;
	  ...
	  if(primerMovimiento && yR == 2 && xR == 0) {  
		  if( tablero[yOrigen][xOrigen].getColor().equals("negra") && tablero[yOrigen + 1][xOrigen].getNombre().equals("Casilla") ||
				  tablero[yOrigen][xOrigen].getColor().equals("blanca") &&  tablero[yOrigen - 1][xOrigen].getNombre().equals("Casilla")  ) {
			  	return true;
		  }
		  return false;
	  }
	  if(yR == 1 && xR == 1) {
		  ...
		  if(tablero[yOrigen][xOrigen].getColor().equals("blanca") && yOrigen > yDestino &&
				  tablero[yDestino][xDestino].getNombre().equals("Casilla") == false  || 
				  tablero[yOrigen][xOrigen].getColor().equals("negra") && yDestino > yOrigen &&
				  tablero[yDestino][xDestino].getNombre().equals("Casilla") == false
				  ) {
			  return true;
		  }
		  ...
		  if(tablero[yDestino][xDestino].getNombre().equals("Casilla") && tablero[yOrigen][xOrigen].getColor().equals("negra") && yOrigen < yDestino 
				  || tablero[yDestino][xDestino].getNombre().equals("Casilla") && tablero[yOrigen][xOrigen].getColor().equals("blanca") && yOrigen > yDestino ) {
			 return true;
  ...
  public void metamorfosisPeon(int yDestino) {
	  if(yDestino == 0 || yDestino == 7) {
		  System.out.println("Porfavor eliga en que se convertira el Peon: \n" +
				  "Rey | Alfil | Torre | Caballo");
		  
		  String buffed = sc.nextLine(),seleccion = sc.nextLine();
		  System.out.println("Has elegido: " + seleccion);
		  fichaCambioLugar();
